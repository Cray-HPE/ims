...
from ims.errors import problemify, generate_missing_input_response, generate_data_validation_failure, \
    generate_resource_not_found_response
from ims.helper import validate_artifact, get_log_id, get_download_url, read_manifest_json
from ims.v2.models.jobs import V2JobRecordInputSchema, V2JobRecordSchema, V2JobRecordPatchSchema
...
class V2BaseJobResource(Resource):
    ...
    def _delete_istio_destination_rule_for_job(self, log_id, job):
        ...
        try:
            api_response = self._delete_namespaced_destination_rule(namespace)(name, body)
    ...
    def create_kubernetes_resources(self, log_id, new_job, template_params):
        """
        ...
        for resource in ("configmap", "service", "job"):
            ...
            try:
                input_file_name = "/mnt/ims/v2/job_templates/{0}/image_{1}_{0}.yaml.template".format(
                    new_job.job_type, resource
                )
                ...
                with open(input_file_name, 'r') as inf, open(output_file_name, 'w') as outf:
                    ...
                    setattr(new_job, resource_field, yaml.load(template_data)["metadata"]["name"])
        ...
        try:
            ...
            current_app.logger.warning("%s Error encountered creating istio DestinationRule %s",
                                       log_id, new_job.kubernetes_job, exc_info=True)
            return None, problemify(status=http.client.INTERNAL_SERVER_ERROR,
    ...
    def delete_kubernetes_resources(self, log_id, job):
        ...
        try:
            ...
            current_app.logger.error("%s Received Exception deleting k8s DestinationRule %s.",
                                     log_id, name, exc_info=True)
            errors.append(str(exception))
            retval = False
...
class V2JobCollection(V2BaseJobResource):
    ...
    def get_artifact_info(job_type, log_id, artifact_id):
        ...
        def _get_recipe_artifact_link():
            ...
            return recipe_record.link, None

        def _get_image_artifact_link():
            ...
            return image_record.link, None

        artifact_link, problem = {
            "create": _get_recipe_artifact_link,
            "customize": _get_image_artifact_link
        }.get(job_type.lower())()

        if problem:
            return None, problem

        md5sum, problem = validate_artifact(artifact_link)
        if problem:
            return None, problem

        return {'link': artifact_link, 'md5sum': md5sum}, None
            ...
            if not root_fs_artifacts:
                ...
                return None, problemify(status=http.client.BAD_REQUEST,
                                        detail='Error reading the manifest.json for IMS image_id={}. The manifest '
                                               'does not include any rootfs artifacts. Determine the specific information '
    ...
    @staticmethod
    def get_download_info(job_type, log_id, artifact_id):

        def _get_recipe_download_url(artifact_info):
            """
            ...
            """
            md5sum = artifact_info["md5sum"] if artifact_info["link"] else ""
            download_url, problem = get_download_url(artifact_info["link"])
            if problem:
                return None, problem

            return {"url": download_url, "md5sum": md5sum}, None

        def _get_image_download_url(artifact_info):
            """
            ...
            """
            manifest_json, problem = read_manifest_json(artifact_info["link"])
            if problem:
                return None, problem
        ...
        artifact_info, problem = V2JobCollection.get_artifact_info(job_type, log_id, artifact_id)
        if problem:
            current_app.logger.info("%s Could not validate artifact or artifact doesn't exist", log_id)
            return None, problem

        ret_val = {
            "create": _get_recipe_download_url,
            "customize": _get_image_download_url
    ...
    def post(self):
        ...
        elif new_job.job_type == "customize":
        ...
        # The download info consists of a download URL and the md5sum (if available) of the file.
        download_info, problem = V2JobCollection.get_download_info(new_job.job_type, log_id, new_job.artifact_id)
        if problem:
        ...
        template_params = {
            ...
            "download_md5sum": download_info["md5sum"],
            ...
            "s3_host": current_app.config['S3_PROTOCOL'] + "://" + current_app.config["S3_ENDPOINT"],
            "s3_bucket": current_app.config["S3_BOOT_IMAGES_BUCKET"]
        }

        new_job, problem = self.create_kubernetes_resources(log_id, new_job, template_params)
        if problem:
            return problem
    ...
    def delete(self):
        ...
        try:
            ...
            for _, job in list(current_app.data['jobs'].items()):
                current_app.logger.info("%s Deleting k8s resources for job_id=%s", log_id, job.id)
